...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    def labels_to_image_weights(labels, nc=80, class_weights=np.ones(80)):
        ...
        class_counts = np.array([np.bincount(x[:, 0].astype(int), minlength=nc) for x in
                                 labels])  # 统计每张图片中的类别个数， 如 array([0, 0, 0, 0, 1, 0, 2, 0, 1]) 2 表示该图片内有该类别的目标两个，COCO 数据集的话长度为 80
        image_weights = (class_weights.reshape(1, nc) * class_counts).sum(
            1)  # 传入的 class_weights 乘以 类别权重，得到每张图像的权重，也就是共 图像个数 个值。 sum(1) 的意思是 按该图片内的所有某个类别的目标乘以类别权重，再求和，就得到每张图像的权重是多少，权重越大，越可能被采样到。
        ...
        if opt.image_weights:
            ...
            if RANK in [-1, 0]:
                cw = model.class_weights.cpu().numpy() * (
                            1 - maps) ** 2 / nc  # class weights # maps为80个0，所以这里是求平方再除以 80
